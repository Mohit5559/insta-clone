...
export class LoginComponent implements OnInit {
  ...
  facebookLogin(){
    ...
    this._apiDataService.getApi('auth/facebook', {}).subscribe({
      ...
      next :(response:any) => {
        ...
        if(response.status === 'success'){
          ...
          // Call sucess toaster.
          // this._toaster.toasterStatus (['success', response.message]);
          ...
          if(response.status === 'fail' && response.data === null){
            // Call fail toaster.
            // this._toaster.toasterStatus (['error', response.message]);
            console.log(response);            
          }
        }
      }
  ...
  googleLogin(){
    // Call api for authorisation.
    this._apiDataService.getApi('auth/google', {}).subscribe({
      // next() method will be executed only when there will be no error.
      next :(response:any) => {
        // On success.
        if(response.status === 'success'){
